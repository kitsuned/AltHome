export class JsonTransformer {
	...
	public constructor(definitions: Record<string, string>) {
		...
		this.definitions = Object.entries(definitions).reduce((accumulator, [key, value]) => ({
			...accumulator,
			[`@${key}@`]: value,
		}), {});
	}
	...
	private reviver<T extends Record<string, any>>(objectScope: T, key: keyof T, value: any): any {
		const shouldReplaceKey = Object.keys(this.definitions).some(v => (key as string).includes(v));
